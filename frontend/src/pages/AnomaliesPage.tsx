    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1">
          Detect outliers, erroneous transactions, and irregular patterns with Isolation Forest, Z-Score, or IQR
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5 space-y-4">
        <div className="flex items-center gap-2 pb-2 border-b border-slate-800">
          ...
          <span className="text-xs font-semibold text-slate-200 uppercase tracking-wider">Detection Parameters</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
          <div>
            <label className="block text-slate-300 font-medium mb-1">Detection Algorithm</label>
            <select
              value={method}
              onChange={(e) => setMethod(e.target.value)}
              className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-slate-200"
            >
              <option value="Isolation Forest">Isolation Forest (Multivariate)</option>
          ...
          {method === 'Z-Score' && (
            <div>
              <label className="block text-slate-300 font-medium mb-1">Z-Score Threshold (σ): {threshold}</label>
              <input
                type="range"
                min="1.5"
                max="5.0"
          ...
          {method === 'IQR' && (
            <div>
              <label className="block text-slate-300 font-medium mb-1">IQR Factor: {factor}x</label>
              <input
                type="range"
                min="1.0"
                max="3.0"
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-semibold text-slate-200">Flagged Anomalous Records</h3>
                <p className="text-xs text-slate-400">Inspecting records deviating significantly from expected patterns</p>
              </div>
              <a
